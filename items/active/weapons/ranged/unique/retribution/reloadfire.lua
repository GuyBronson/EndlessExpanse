...
function ReloadFire:update(dt, fireMode, shiftHeld)
	...
	setupHelper(self, {"reloadfire-update", "reloadfire-auto", "reloadfire-postauto", "reloadfire-burst", "reloadfire-postburst"})
	if self.helper then
		self.helper:runScripts("reloadfire-update", self, dt, fireMode, shiftHeld)
	end
...
-- ***********************************************************************************************************
	...
	if self.helper then
		self.helper:runScripts("reloadfire-auto", self)
	end
	...
	if self.helper then self.helper:runScripts("reloadfire-postauto", self) end
...
function ReloadFire:burst()
	...
	if self.helper then
		self.helper:runScripts("reloadfire-burst", self)
	end
	...
	if self.helper then self.helper:runScripts("reloadfire-postburst", self) end
end

function ReloadFire:cooldown()
...
function ReloadFire:muzzleFlash()
...
function ReloadFire:fireProjectile(projectileType, projectileParams, inaccuracy, firePosition, projectileCount)
...
function ReloadFire:firePosition()
...
function ReloadFire:energyPerShot()
...
function ReloadFire:isResetting()
...
function ReloadFire:isChargeUp()
	self.isCrossbow = config.getParameter("isCrossbow",0) -- is this a crossbow?
	self.isSniper = config.getParameter("isSniper",0) -- is this a sniper rifle?
...
function ReloadFire:hasShotgunReload()
	self.isReloader = config.getParameter("isReloader",0) -- is this a shotgun style reload?
...
function ReloadFire:checkAmmo(force)
...
function ReloadFire:checkMagazine(evalOnly)
	self:calcAmmo()
...
function ReloadFire:applyRecoil()
	--Recoil here
...
function ReloadFire:adjustRecoil()		-- if we are not grounded, we halve the force of the recoil
